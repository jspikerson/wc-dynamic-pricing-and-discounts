...
jQuery(document).ready(function() {
    ...
    function duplicate_row(key, row)
    {
        ...
        if (row.find('.rp_wcdpd_' + key + '_unchanged').length) {
            add_row_content(key, row);
        }
    ...
    function fix_meta_field_condition(key, alias, element, condition_type)
    {
        ...
        if (condition_type !== 'customer__meta' && condition_type !== 'product_property__meta' && condition_type !== 'product_other__cart_item_data') {
            return;
        }
    ...
    function initialize_select2_multiselect(key, element, type)
    {
        ...
        var config = {
            width: '100%',
            minimumInputLength: (element.hasClass('rp_wcdpd_condition_weekdays') || element.hasClass('rp_wcdpd_condition_days_of_month') || element.hasClass('rp_wcdpd_condition_months')) ? 0 : 1,
    ...
    function fix_row_header(element, key, config)
    {
        ...
        // Set title
        row.find('.rp_wcdpd_row_title_title').html(jQuery.rightpress.escape_html(title)).css('display', 'inline-block');
        ...
        if (key !== 'product_pricing') {
            ...
            // Set note
            row.find('.rp_wcdpd_row_title_note').html(jQuery.rightpress.escape_html(note)).css('display', (note === '' ? 'none' : 'inline-block'));
        }
        ...
        if (key === 'product_pricing') {
            ...
            // Display method text
            row.find('.rp_wcdpd_row_title_method').html(jQuery.rightpress.escape_html(method_text)).css('display', 'inline-block');
        }
        ...
        if (typeof pricing_method !== 'undefined' && pricing_method !== '' && typeof pricing_value !== 'undefined' && pricing_value !== '' && jQuery.isNumeric(pricing_value)) {
            ...
            row.find('.rp_wcdpd_row_title_pricing').html(jQuery.rightpress.escape_html(pricing_string)).css('display', 'inline-block');
        }
