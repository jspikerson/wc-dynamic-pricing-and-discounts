...
final class RightPress_Help
{
    ...
    public static function is_request($type)
    {
        ...
        switch ($type)
        {
            ...
            case 'frontend':
                return ((!is_admin() || (defined('DOING_AJAX') && !isset($_REQUEST['woocommerce_quick_edit']))) && !defined('DOING_CRON'));
            default:
    ...
    public static function get_wc_order_id($order_id = null)
    {
        ...
        if (is_admin() && wp_doing_ajax() && isset($_REQUEST['action']) && in_array($_REQUEST['action'], array('woocommerce_load_order_items', 'woocommerce_save_order_items', 'woocommerce_calc_line_taxes', 'woocommerce_get_order_details'), true) && !empty($_REQUEST['order_id'])) {
     ...
     * @return array
     */
    public static function get_wc_product_attribute_ids($product, $selected = array())
    {
        $attribute_ids = array();
     ...
     * @return float
     */
    public static function calculate_subtotal($include_tax = false, $cart_items = null)
    {

        $subtotal = 0.0;

        // Get cart items
        $cart_items = $cart_items !== null ? $cart_items : RightPress_Help::get_wc_cart_items();
     ...
     * @return int
     */
    public static function get_wc_cart_sum_of_item_quantities($params = array())
    {

        $sum = 0;

        // Get cart items
        $cart_items = !empty($params['cart_items']) ? $params['cart_items'] : RightPress_Help::get_wc_cart_items();
     ...
     * Get months
     *
     * Numeric indexes - 1 stands for January etc.
     *
     * @access public
     * @param bool $localize
     * @return array
     */
    public static function get_months($localize = true)
    {

        $months = [];

        // Iterate over month numbers
        for ($i = 1; $i <= 12; $i++) {
     ...
     * @return string
     */
    public static function get_literal_end_of_week()
    {

        return strtolower(date('l', strtotime(RightPress_Help::get_literal_start_of_week() . ' -1 day')));
    }
     ...
     *
     * Optional parameters:
     *   date           object          Date from object
     *   date_to        object          Date to object
    ...
    public static function get_wc_order_ids($params = array())
    {
        ...
        if (!empty($params['date']) && is_object($params['date'])) {
            ...
            // Date to is set
            if (!empty($params['date_to']) && is_object($params['date_to'])) {

                $config['date_created'] = $params['date']->format('U') . '...' . $params['date_to']->format('U');
            }
            // Date to is not set
            else {
